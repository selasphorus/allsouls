...
function birdhive_sanitize ( $str = null ) {
    return $str;
}
...
function get_title_uid ( $post_id = null, $post_type = null, $tmp_title = null, $uid_field = 'title_for_matching' ) {
    ...
    birdhive_log( "divline2" );
    birdhive_log( "function called: get_title_uid" );
    
    birdhive_log( "[get_title_uid] post_id: ".$post_id."; post_type: ".$post_type );
    birdhive_log( "[get_title_uid] tmp_title: ".$tmp_title ); // formerly post_title
    birdhive_log( "[get_title_uid] uid_field: ".$uid_field );
    ...
    //birdhive_log( "old_title: ".$post_title );
    birdhive_log( "[get_title_uid] old_t4m: ".$old_t4m );    
    ...
    if ( $t4m_posts && $t4m_posts > 0 ) { // meta_value_exists( $post_type, $meta_key, $meta_value )
        ...
        // not unique! fix it...
        birdhive_log( "[get_title_uid] new_t4m not unique! Fix it.");
    ...
    if ( $new_t4m == $old_t4m ) {
        birdhive_log( "[get_title_uid] new_t4m same as old_t4m." );
    } else {
        birdhive_log( "[get_title_uid] new_t4m: ".$new_t4m );
    }
...
function update_title_for_matching ( $post_id ) {
    ...
    birdhive_log( "function: update_title_for_matching" );
    ...
    /*
    // tft
    // array of rep post_ids for testing
    $ids = array('146085', '19858', '149771', '123988');
    ...
    */
    ...
    if ( ! $title_clean || $title_clean == "" ) {
        birdhive_log( "No title_clean stored in post_meta for this post -> use make_clean_title" );
    ...
    if ( !empty($new_t4m) ) {
        ...
        if ( update_post_meta( $post_id, 'title_for_matching', $new_t4m ) ) {
            $info .= birdhive_add_post_term( $post_id, 't4m-updated', 'admin_tag', true ); // $post_id, $arr_term_slugs, $taxonomy, $return_info
        }
...
function build_the_title( $post_id = null, $uid_field = 'title_for_matching', $arr = array(), $abbr = false ) {
    ...
    birdhive_log( "divline2" );
    birdhive_log( "function called: build_the_title" );
    
    birdhive_log( "[btt] post_id: ".$post_id );
    birdhive_log( "[btt] abbr: ".(int)$abbr );
    ...
    if ( !empty($arr) ) {
        ...
        birdhive_log( "running btt using array derived from _POST." );
        ...
        if ( $post_type == 'repertoire' ) {
            ...
            $musical_work_id = $arr['musical_work'];
            
            // TODO: turn this array into a string!
            $choir_forces = $arr['choir_forces'];
            if ( is_array($choir_forces) ) {
                birdhive_log( "[btt/edition] no voicings or soloists info >> use choir_forces_str: ".print_r($choir_forces, true) );
            ...
            //birdhive_log( "editors: ".print_r($editors, true) );
            $editors_str = str_from_persons_array ( $editors, 'editors', $post_id, $format, $arr_of, $abbr );
            //birdhive_log( "editors_str: ".$editors_str );
            birdhive_log( "-----");
            
            $publication_id = $arr['publication']; // single id
            //birdhive_log( "[btt/arr] publication_id: ".$publication_id ); // tft
            if ( $publication_id ) {
            ...
            //birdhive_log( "[btt/arr] publisher_id: ".$publisher_id ); // tft
            if ( $publisher_id ) {
            ...
            $voicings_str = "";
            if ( count($voicings) > 0 ) {
                //birdhive_log( "[btt/arr] voicings: ".print_r($voicings, true) );
                foreach ( $voicings as $voicing_id ) {
                    if ( $voicing_id != 0) {
                        $term = get_term( $voicing_id );
                        if ($term) { 
                            //birdhive_log( "[btt] voicing_id: $voicing_id/ term->name: ".$term->name );
                ...
                //birdhive_log( "[btt/arr] voicings_str: ".$voicings_str );
            }
            if ( $voicings_str == "" ) {
                //birdhive_log( "[btt/arr] voicings_str is empty." );
                if ( $arr['voicing_txt'] != "" ) {
                    $voicings_str = $arr['voicing_txt'];
                    birdhive_log( "[btt/arr] using backup txt field for voicings_str" );
                }
            } else {
                //birdhive_log( "[btt/arr] voicings_str: ".$voicings_str );
            }
            ...
            if ( $soloists_str == "" && $arr['soloists_txt'] != "" ) {
                ...
                birdhive_log( "[btt/arr] using backup txt field for soloists_str" );
            }
            ...
            if ( $instruments_str == "" && $arr['instrumentation_txt'] != "" ) {
                ...
                birdhive_log( "[btt/arr] using backup txt field for instruments_str: ".$instruments_str );
            }
        ...
        if ( $keys_str == "" && $arr['key_name_txt'] != "" ) {
            ...
            birdhive_log( "[btt/arr] using backup txt field for keys_str" );
        }
        ...
        birdhive_log( "running btt based on post_id." );
        ...
        if ( $post_type == 'repertoire' ) {
            ...
            // ACF -- WIP
            $musical_work = get_field('musical_work', $post_id);
            //$musical_work_id = ...
            //
            $editors_str = "";
            ...
            foreach ( $editors as $editor ) {
                ...
                if ( $editor_id ) {
                    ...
                    if (count($editors) > 1) { $editors_str .= ", "; }
                }
            }
            // Trim trailing comma and space
    ...
    if ( empty($arr) && $post_id && ( $post_type == 'repertoire' || $post_type == 'edition' ) ) {
        ...
        birdhive_log( "[btt] get taxonomy info from post_id: ".$post_id ); // tft
        ...
        birdhive_log( "[btt/taxonomies] keys_str: ".$keys_str ); // tft
        ...
        if ( $post_type == 'edition' ) {
            ...
            birdhive_log( "[btt/taxonomies] voicings_str: ".$voicings_str ); // tft
    ...
    if ( $post_type == 'repertoire' ) {
        ...
        if ( in_array($hymn_cat_id, $rep_categories) ) {
            ...
            birdhive_log( "[btt] This is a hymn." ); // tft
        ...
        if ( $musical_work_id != null ) {
            birdhive_log( "[btt] musical_work_id: ".$musical_work_id );
            
            $musical_work_title = get_field('post_title', $musical_work_id);
            
            // Get abbreviated version of work title w/ lastnames only
            
            // MW Authorship
            
            // A. Short version
            $rep_authorship_short = get_authorship_info( array( 'post_id' => $musical_work_id ), 'edition_title', true ); // abbr
            //birdhive_log( "[btt/edition] rep_authorship_short: ".$rep_authorship_short ); // tft
            ...
            //birdhive_log( "[btt/edition] rep_authorship_long: ".$rep_authorship_long ); // tft
            ...
            birdhive_log( "[btt/edition] rep_authorship_long: ".$rep_authorship_long ); // tft
            birdhive_log( "[btt/edition] musical_work_title: ".$musical_work_title ); // tft
            
            // Compare short/long authorship; replace as needed
            if ( $rep_authorship_long != $rep_authorship_short ) {
                birdhive_log( "[btt/edition] replace long with short authorship info." ); // tft
                ...
                birdhive_log( "[btt/edition] num replacements made: ".$count." (Replace [".$rep_authorship_long."] with [".$rep_authorship_short."] in [".$musical_work_title."] )" ); // tft
                birdhive_log( "[btt/edition] revised musical_work_title: ".$musical_work ); // tft
            } else {
                birdhive_log( "[btt/edition] rep_authorship_long same as rep_authorship_short." ); // tft
                $musical_work = $musical_work_title;
            }
            $new_title .= $musical_work;
            //birdhive_log( "[btt/edition] revised musical_work_title: ".$musical_work ); // tft
            //birdhive_log( "[btt/edition] new_title (after adding musical_work info): ".$new_title );
        } else {
            birdhive_log( "[btt/edition] Abort! No musical_work found upon which to build the title." );
        ...
        //birdhive_log( "[btt/edition] new_title (after editors_str): ".$new_title );
        
        // 4. Publication
        if ( $publication != "" ) { $new_title .= " / ".$publication; }
        //birdhive_log( "[btt/edition] new_title (after publication): ".$new_title );
        
        // 5. Publisher
        if ( $publisher != "" ) { $new_title .= " / ".$publisher; }
        //birdhive_log( "[btt/edition] new_title (after publisher): ".$new_title );
        
        // 6. Voicings
        //birdhive_log( "[btt/edition] voicings_str: ".$voicings_str ); // tft
        if ( $voicings_str == "" ) { 
            //birdhive_log( "[btt/edition] voicings_str is empty >> try voicing_txt." ); // tft
            if ( $voicing_txt != "" ) {
                $voicings_str = $voicing_txt;
                //birdhive_log( "[btt/edition] voicings_str from voicing_txt: ".$voicings_str ); // tft
            } else {
                //birdhive_log( "[btt/edition] no luck w/ voicing_txt >> voicings_str still empty: [".$voicings_str."]" ); // tft
            }
        }
        if ( $voicings_str !== "" && $uid_field == 'title_for_matching' ) { 
            //birdhive_log( "[btt/edition] add voicings_str to new_title." ); // tft
            $new_title .= " / for ".$voicings_str;
        }
        //birdhive_log( "[btt/edition] new_title (after voicings_str): ".$new_title );
        
        // (if no voicing, but yes soloists, then "for soloists")
        // multiple voicings &c. -- connect w/ ampersand?
        
        // 7. Soloists
        //birdhive_log( "[btt/edition] soloists_str: ".$soloists_str ); // tft
        ...
        //birdhive_log( "[btt/edition] new_title (after soloists_str): ".$new_title );
        
        // Choir Forces -- if no Voicings or Soloists
        if ( $choir_forces_str != "" & $voicings_str == "" && $soloists_str == "" && $uid_field == 'title_for_matching' ) {
            birdhive_log( "[btt/edition] no voicings or soloists info >> use choir_forces_str: ".$choir_forces_str );
            $new_title .= " / for ".$choir_forces_str;
        } else {
            birdhive_log( "[btt/edition] choir_forces: ".$choir_forces_str );
        }
        
        // 8. Instrumentation
        //birdhive_log( "[btt/edition] instruments_str: ".$instruments_str ); // tft
        ...
        //birdhive_log( "[btt/edition] new_title (after instruments_str): ".$new_title );
        
        // 9. Box num
        if ( $box_num != "" ) { $new_title .= " [in ".$box_num."]"; }
        //birdhive_log( "[btt/edition] new_title (after box_num): ".$new_title );
        
    }
    
    birdhive_log( "[btt] new_title (final): ".$new_title );
...
function build_the_title_on_insert( $data, $postarr ) {
    ...
    birdhive_log( "divline1" );
    birdhive_log( "filter: wp_insert_post_data" );
    birdhive_log( "function called: build_the_title_on_insert" );
    ...
    birdhive_log( "post_id: ".$post_id."; post_type: ".$post_type );
    ...
    //birdhive_log( "data: ".print_r($data, true) ); // tft
    //birdhive_log( "postarr: ".print_r($postarr, true) ); // tft
    //birdhive_log( "_POST array: ".print_r($_POST, true) ); // tft
    
    if ( $post_type == 'repertoire' && isset($_POST['acf']['field_624615e7eca6f']) ) {
        
        birdhive_log( "build the array of repertoire _POST data for submission to fcn build_the_title." );
    ...
    } else if ( $data['post_type'] == 'edition' && isset($_POST['acf']['field_626811b538d8e']) ) { //  
        
        birdhive_log( "build the array of edition _POST data for submission to fcn build_the_title." );
        ...
        birdhive_log( "Insufficient data for building the array of _POST data for submission to fcn build_the_title." );
    ...
    if ( $new_title ) {
        ...
        if ( strpos($new_title, 'Error! Abort!') !== false ) {
            ...
            birdhive_log( $new_title );
            
        } else if ( $new_title == $data['post_title'] ) {
            
            birdhive_log( "new_title same as old_title.");
            
        } else {

            birdhive_log( "old_title: ".$data['post_title'] );
            birdhive_log( "new_title: ".$new_title );
...
function remove_bracketed_info ( $str ) {
    ...
    birdhive_log( "function: remove_bracketed_info" );
...
function filter_the_title( $post_title, $post_id = null ) {
    ...
    //birdhive_log( "function: filter_the_title" );
    $post_type = ""; // init
...
function make_clean_title( $post_id = null, $post_title = null, $return_revised = true ) {
    ...
    birdhive_log( "divline1" );
    birdhive_log( "function: make_clean_title");
    ...
    birdhive_log( "post_id: ".$post_id."; post_title: ".$post_title );
    
    if ( $return_revised == true ) {
        birdhive_log( "function: make_clean_title >> return_revised" );
    }
...
function clean_slug( $post_id ) {
    ...
    birdhive_log( "divline1" );
    birdhive_log( "function: clean_slug");
    ...
    // Clean title
    $title_clean = get_post_meta( $post_id, 'title_clean', true );
    //birdhive_log( "title_clean (get_post_meta): ".$title_clean);
    if ( empty ($title_clean) || strpos($title_clean, '[') !== false ) { 
        $title_clean = make_clean_title( $post_id );
        //birdhive_log( "title_clean (make_clean_title): ".$title_clean);    
    }
    birdhive_log( "title_clean: ".$title_clean);
    ...
    if ( preg_match($match_pattern, $title_clean) ) {
        ...
        $append_date = false;
        birdhive_log( "append_date FALSE");
    } else {
        birdhive_log( "append_date TRUE");
...
///add_action( 'save_post', 'birdhive_save_post_callback', 10, 3 );
function birdhive_save_post_callback( $post_id, $post, $update ) {
    
    birdhive_log( "divline1" );
    birdhive_log( "action: save_post" );
    //birdhive_log( "action: save_post_event" );
    birdhive_log( "function called: birdhive_save_post_callback" );
    
    if ( is_dev_site() ) {
        birdhive_add_post_term( $post_id, 'dev-test-tmp', 'admin_tag', true ); // tft
    }
    ...
    if ( wp_is_post_revision( $post_id ) ) { 
        birdhive_log( "is post revision >> abort." );
        return;
    }
    ...
    if ( $post_type == "event" ) {
        ...
        if ( $recurrence_id ) { 
            birdhive_log( "is part of recurring event series ($recurrence_id) >> abort." );
            return;
        }
        ...
        //birdhive_run_post_updates( $atts = [] )
        //$info = birdhive_run_post_updates( array ( 'post_id' => $post_id ) )
        ...
        birdhive_log( "old_slug: ".$old_slug);
        birdhive_log( "new_slug: ".$new_slug);

        // Check to see if new_slug is really new. If it's identical to the existing slug, skip the update process.
        if ( $new_slug != $old_slug ) {

            // unhook this function to prevent infinite looping
            remove_action( 'save_post', 'birdhive_save_post_callback' );
            //remove_action( 'save_post_event', 'birdhive_save_event_post_callback' );
            ...
            if ( ! is_wp_error($post_id) ) {
                // If update was successful, add admin tag to note that slug has been updated
                birdhive_add_post_term( $post_id, 'slug-updated', 'admin_tag', true ); // $post_id, $arr_term_slugs, $taxonomy, $return_info
                //$info .= birdhive_add_post_term( $post_id, 'slug-updated', 'admin_tag', true ); // $post_id, $arr_term_slugs, $taxonomy, $return_info
            }

            // re-hook this function
            add_action( 'save_post', 'birdhive_save_post_callback', 10, 3 );
            //add_action( 'save_post_event', 'birdhive_save_event_post_callback' );
        ...
        birdhive_log( "about to call function: get_title_uid" );
        $new_t4m = get_title_uid( $post_id, $post_type, $post_title );
        
        birdhive_log( "divline2" );

        birdhive_log( "post_id: ".$post_id );
        birdhive_log( "post_type: ".$post_type );
        birdhive_log( "post_title: ".$post_title );
        //birdhive_log( "old_title: ".$old_title );
        birdhive_log( "old_t4m: ".$old_t4m );
        //birdhive_log( "new_title: ".$new_title );
        birdhive_log( "new_t4m: ".$new_t4m );
        ...
        if ( !empty($new_t4m) && $new_t4m != $old_t4m && $new_t4m != "test") {
            ...
            birdhive_log( ">> update t4m" );
            //$update_data['meta_input'] = array( 'title_for_matching' => $new_t4m );
            $meta_update = update_post_meta( $post_id, 'title_for_matching', $new_t4m );
            birdhive_log( "meta_update: ".$meta_update );
            if ( $meta_update === true ) {
                birdhive_log("success!");
                //birdhive_log(birdhive_add_post_term( $post_id, array('t4m-updated', 'programmatically-updated'), 'admin_tag', true ));
            } else {
        ...
        if ( empty($title_for_matching) ) {
            $t4m_info = update_title_for_matching( $post_id ); // stc custom function
