	...
	if ( count($rows) > 0 ) {
        ...
        foreach ($rows as $row) {
            ...
            if ( $delete_row == true ) {
                ...
                //birdhive_log("personnel row to be deleted:");
                //birdhive_log( print_r($row, true) );
                ...
                if ( $deletion_count == 0 && $do_deletions == true) {
                    ...
                    if ( delete_row('personnel', $i, $post_id) ) { // ACF function: https://www.advancedcustomfields.com/resources/delete_row/ -- syntax: delete_row($selector, $row_num, $post_id) 
                        ...
                        $deletion_count++;
                        //birdhive_log("[personnel row $i deleted successfully]");
                    } else {
                        $row_info .= "<!-- [deletion failed for personnel row $i] -->";
                        //birdhive_log("[failed to delete personnel row $i]");
                    }
                    ...
                    if ( $do_deletions == true ) {
                        ...
                        //birdhive_log("row to be deleted on next round due to row_index issues.");
                    } else {
                        $row_info .= "<!-- [$i] row to be deleted when do_deletions is re-enabled. -->";
                        //birdhive_log("row to be deleted when do_deletions is re-enabled.");
                    }
        ...
        //if ( $placeholder_label == false && $placeholder_item == false ) { 
            //$row_info .= birdhive_remove_post_term( $post_id, 'program-personnel-placeholders', 'admin_tag', true ); 
        //
...
function get_personnel_role ( $a = array() ) {
	...
	if ( empty($person_role) ) {
		...
		if ( $placeholder_label == true && $run_updates == true  ) { 
			...
			$info .= birdhive_add_post_term( $post_id, 'program-personnel-placeholders', 'admin_tag', true ); // $post_id, $arr_term_slugs, $taxonomy, $return_info
		}
    ...
    if ( count($rows) > 0 ) {
        ...
        foreach( $rows as $row ) {
            ...
            if ( $delete_row == true ) {
                ...
                //birdhive_log("divline1");
                //birdhive_log("program row to be deleted:");
                //birdhive_log( print_r($row, true) );
                ...
                if ( $deletion_count == 0 && $do_deletions == true ) {
                    ...
                    if ( delete_row('program_items', $i, $post_id) ) { // ACF function: https://www.advancedcustomfields.com/resources/delete_row/ -- syntax: delete_row($selector, $row_num, $post_id) 
                        ...
                        $deletion_count++;
                        //birdhive_log("[program row $i deleted successfully]");
                    } else {
                        $row_info .= "<!-- [deletion failed for program row $i] -->";
                        //birdhive_log("[failed to delete program row $i]");
                    }
...
function get_program_item_label ( $a = array() ) {
	...
	if ( !empty ($item_label)) {
		...
		if ( ($label_update_required == true || $placeholder_label == true) && $run_updates == true ) {
			...
			$info .= birdhive_add_post_term( $post_id, 'program-item-placeholders', 'admin_tag', true ); // $post_id, $arr_term_slugs, $taxonomy, $return_info
		}
            ...
            if ( $program_item_obj_id ) {
                ...
                if ( $item_post_type == 'repertoire' ) {
                    ...
                    if ( $row_type == 'title_only' ) {
                        ...
                        $item_name = get_rep_info( $program_item_obj_id, 'display', true, true ); // item name with authorship info
                        ...
                        $item_name = get_authorship_info( array( 'post_id' => $program_item_obj_id ), 'concert_item', false, false, false ); // get_authorship_info( $data = array(), $format = 'post_title', $abbr = false, $is_single_work = false, $show_title = true ) {
    ...
    if ( $posts ) {
        ...
        foreach ( $posts AS $post ) {
            ...
            if ( count($arr_repeater_rows_indices) != $num_repeater_rows ) {
                ...
                $post_info .= birdhive_add_post_term( $post_id, 'cleanup-required', 'admin_tag', true ); // $post_id, $arr_term_slugs, $taxonomy, $return_info
            }
...
add_filter('em_event_output_placeholder','birdhive_placeholders',1,3);
function birdhive_placeholders( $replace, $EM_Event, $result ) {
    ...
    if ( $result == '#_EVENTLINK' ) {
        ...
        if ( $show_image == true ) {
            ...
            $replace .= $caption."<!-- birdhive_placeholders -->";
            ...
            $replace .= "<br /><!-- stc-calendar >> birdhive_placeholders -->"; // If there's no featured image, add a line break to keep the spacing
        }
...
function em_args_mod($args){
    ...
    //birdhive_log("em_events_output_grouped_args: ".print_r($args, true));
	return $args;
}


// Create custom scopes: "Upcoming", "This Week", "This Season", "Next Season", "This Year", "Next Year"
function birdhive_em_custom_scopes( $scope = null ) {
...
//add_filter( 'em_object_build_sql_conditions_args', 'birdhive_em_custom_scope_arg',10,1); // CMS(?)
function birdhive_em_custom_scope_arg( $args = array() ){

	if( is_admin() && !empty($args['scope']) ) {
		
		$scope = $args['scope'];
		if ( ! is_array($scope) ) {
			//birdhive_log("args['scope']".$args['scope']);
		} else {
			//birdhive_log("args['scope']". print_r($args['scope'],true) );
		}
		
        // If this is the main admin events page...
        //birdhive_log("args: ". print_r($args, true) );
        //https://dev.saintthomaschurch.org/wp-admin/edit.php?s&post_status=all&post_type=event
        
		// TODO: figure out how to eliminate redundancy of array declaration w/ birdhive_em_scopes
		$my_scopes = array( 'this-season', 'next-season', 'this-year', 'next-year');
		
		if ( in_array($scope, $my_scopes) ) {		
			
			birdhive_log($scope." is a custom scope.");
			$arr_dates = birdhive_em_custom_scopes($scope);
...
add_filter( 'em_get_scopes', 'birdhive_em_scopes', 10, 1);
function birdhive_em_scopes($scopes){
    $my_scopes = array(
		'upcoming' => 'Upcoming',
		'this-week' => 'This Week',
...
add_filter('em_events_get_default_search','birdhive_custom_event_search_parameters',1,2);
add_filter('em_calendar_get_default_search','birdhive_custom_event_search_parameters',1,2);
function birdhive_custom_event_search_parameters($args, $array){
...
add_filter( 'em_events_build_sql_conditions', 'birdhive_custom_event_search_build_sql_conditions',1,2);
function birdhive_custom_event_search_build_sql_conditions($conditions, $args){
    
    birdhive_log( "divline2" );
    birdhive_log( "function called: birdhive_custom_event_search_build_sql_conditions" );
    
    birdhive_log( "[birdhive_custom_event_search...] conditions: ".print_r($conditions, true) );
    birdhive_log( "[birdhive_custom_event_search...] args: ".print_r($args, true) );
    
    global $wpdb;
    
    if( !empty($args['series']) && is_numeric($args['series']) ){
        
        birdhive_log( "[birdhive_custom_event_search...] series is set and valid: ".$args['series'] );
    ...
    if( !empty($args['scope']) ) {
        ...
        birdhive_log( "[birdhive_custom_event_search...] scope: ".print_r( $args['scope'],true ) );
    ...
    birdhive_log( "[birdhive_custom_event_search...] modified conditions: ".print_r($conditions, true) );
